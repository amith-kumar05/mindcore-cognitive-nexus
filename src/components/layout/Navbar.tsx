...
const Navbar = () => {
  ...
  return <>
      <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'py-2 bg-mindcore-darker/80 backdrop-blur-md shadow-md' : 'py-4 bg-transparent'}`}>
        <div className="container mx-auto px-4 flex items-center justify-between">
          ...
          <a href="#" className="flex items-center gap-2 font-orbitron font-bold text-xl text-white">
            ...
            <span className="text-base font-bold">MindCore</span>
          </a>

          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map(link => <div key={link.name} className="relative group">
                <a href={link.href} className="text-mindcore-text-muted hover:text-mindcore-text transition-colors text-sm font-medium flex items-center gap-1" onClick={e => {
              if (link.hasDropdown) {
                e.preventDefault();
                ...
                {link.hasDropdown && <AnimatePresence>
                ...
                duration: 0.2
                        ...
                        {link.dropdown?.map(item => <a key={item.name} href={item.href} className="flex flex-col p-3 rounded-md hover:bg-mindcore-accent/10 transition-all group">
            ...
            <div className="container mx-auto px-4 py-6">
              <nav className="flex flex-col gap-4">
                {navLinks.map(link => <div key={link.name}>
                    <a href={link.href} className="text-mindcore-text font-medium py-2 block" onClick={e => {
                if (link.hasDropdown) {
                  e.preventDefault();
                  toggleDropdown(link.name);
                } else {
                  setIsMobileMenuOpen(false);
                      ...
                      <div className="flex items-center justify-between">
                        <span>{link.name}</span>
                        {link.hasDropdown && <ChevronDown size={18} className={`transition-transform ${activeDropdown === link.name ? 'rotate-180' : ''}`} />}
                      </div>
                    </a>
                  ...
                  height: 0
                            ...
                            {link.dropdown?.map(item => <a key={item.name} href={item.href} className="block py-2 text-sm text-mindcore-text-muted" onClick={() => setIsMobileMenuOpen(false)}>
                                {item.name}
                              </a>)}
                          </motion.div>}
                      </AnimatePresence>}
              ...
              <div className="mt-6">
                <button className="w-full neon-button py-3" onClick={() => setIsMobileMenuOpen(false)}>
                  Get Started
                </button>
              </div>
            </div>
