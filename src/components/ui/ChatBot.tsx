...
const ChatBot = () => {
  ...
  const [messages, setMessages] = useState<{ text: string; isUser: boolean; }[]>([
    { text: "Hello! I'm MindCore Assistant. How can I help you today?", isUser: false }
  ]);
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-mindcore-accent text-black flex items-center justify-center shadow-neon"
      >
          ...
          >
            ...
            <div className="bg-mindcore-dark p-4 border-b border-mindcore-accent/20 flex items-center justify-between">
              ...
              <button 
                ...
                className="text-mindcore-text-muted hover:text-white transition-colors"
              >
                <X size={20} />
              </button>
            </div>
                ...
                >
                  <div 
                    className={`max-w-[80%] p-3 rounded-lg ${
                      message.isUser 
                        ? 'bg-mindcore-accent text-black ml-auto' 
                        : 'bg-mindcore-secondary/40 backdrop-blur-sm text-mindcore-text'
                    }`}
                  >
                    <div className="flex items-start gap-2">
                      {!message.isUser && (
                        <Bot size={16} className="mt-1 min-w-4" />
                      )}
                      <p className="text-sm">{message.text}</p>
                      {message.isUser && (
                        <User size={16} className="mt-1 min-w-4" />
                      )}
                    </div>
                  </div>
                </div>
              ...
              <div className="flex gap-2">
                ...
                <button 
                  ...
                  className={`w-10 h-10 rounded-md flex items-center justify-center transition-colors ${
                    input.trim() 
                      ? 'bg-mindcore-accent text-black' 
                      : 'bg-mindcore-accent/30 text-mindcore-text-muted cursor-not-allowed'
                  }`}
                >
                  <Send size={18} />
                </button>
